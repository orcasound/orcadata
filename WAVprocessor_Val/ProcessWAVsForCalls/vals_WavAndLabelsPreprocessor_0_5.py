...
def getNextArray_new(sampleRate, wavData, wavPtr, nPsdPts, nWavPts4ary):
  ...
  if DEBUG == 1: print("Nfft=",Nfft,"sliceStep=",sliceStep, "num of slices", NrawT, "sampleStep",sampleStep, "wavPtr",wavPtr)
...
def getSpectrogram(target, priorFile, theWavData):
  ...
  if DEBUG == 1:
    ...
    print("Expert startSamples =", target.start_time_s*sampleRate, "windowStart = ", wavPtr, "Expert stop samples",(target.start_time_s + target.duration_s)*sampleRate, "windowStop", wavPtr+nWavPts4ary)
  ...
  if not logFrequencyScale:
    if removeClicks:
      specType= "Signal: normCompressedDb - deClicked"
      title = "{} \n{}\nExpert {:.1f},{:.1f}, Display {:.1f},{:.1f}".format(specType,wavFile,target.start_time_s, target.start_time_s+target.duration_s,wavPtr/sampleRate,(wavPtr + nWavPts4ary)/sampleRate)
    else:
      specType= "Signal: normCompressedDb"
      title = "{}\n{}\nExpert {:.1f},{:.1f}, Display {:.1f},{:.1f}".format(specType,wavFile,target.start_time_s, target.start_time_s+target.duration_s,wavPtr/sampleRate,(wavPtr + nWavPts4ary)/sampleRate)
  else:
    if removeClicks:
      specType= "Signal: normBandedDb - deClicked"
      title = "{} \n{}\nExpert {:.1f},{:.1f}, Display {:.1f},{:.1f}".format(specType,wavFile,target.start_time_s, target.start_time_s+target.duration_s,wavPtr/sampleRate,(wavPtr + nWavPts4ary)/sampleRate)
    else:
      specType= "Signal: normBandedDb"
      title = "{}\n{}\nExpert {:.1f},{:.1f}, Display {:.1f},{:.1f}".format(specType,wavFile,target.start_time_s, target.start_time_s+target.duration_s,wavPtr/sampleRate,(wavPtr + nWavPts4ary)/sampleRate)
  ...
  for cnt, line in enumerate(elf):
    ...
    if cnt>0:  # skip header line
      ...
      if items[2] != '0':  # skip any experts' record which has a duration_s value = ZERO
                           # thisClass, flow, fhigh, dt, rmclicks, logScale, thisDate
        expertLabelList.append(acousticLabel(line, Nfft, 0, thisExpertClass, n_bands, n_slices, f_low, f_high, deltaT, removeClicks, logFrequencyScale,rightNow))
...
for cnt, exLabel in enumerate(expertLabelList):   ##########  Main processing loop ####################################
  ...
  signalArrayList.append(theSignal)
  signalLabelList.append(signalLabel)
...
if DEBUG == 1: 
  ...
  for cnt, sig in enumerate(signalLabelList):
    print(sig.wav_filename,sig.window_start_idx, sig.window_stop_idx)
  
os.chdir(pklDirectory) 
loc = wavFileDirectory.split('/')[-2]
if not logFrequencyScale:
  save_obj(signalArrayList, "signalArrayList_%dx%d_%dto%d_%0.1fsec" % (n_bands,n_slices,f_low,f_high,deltaT))
  save_obj(signalLabelList, "signalLabelList_%dx%d_%dto%d_%0.1fsec" % (n_bands,n_slices,f_low,f_high,deltaT))
else:
  save_obj(signalArrayList, "signalArrayList_%dx%d_%dto%d_%0.1fsec_logF" % (n_bands,n_slices,f_low,f_high,deltaT))
  save_obj(signalLabelList, "signalLabelList_%dx%d_%dto%d_%0.1fsec_logF" % (n_bands,n_slices,f_low,f_high,deltaT))  

print("Number of events: signals",len(signalArrayList))
